...
namespace VStage.Networking
{
    ...
    public class VRIKNetworkPlayer : NetworkBehaviour
    {
        ...
        [SerializeField] private SimpleShinanoFacialTracking_Host hostFacialTracking;
        [SerializeField] private SimpleShinanoFacialTracking_Client clientFacialTracking;
        
        // 현재 활성화된 페이셜 트래킹 컴포넌트에 대한 공통 인터페이스
        private object activeFacialComponent;
        ...
        public override void Spawned()
        {
            ...
            else
            {
                ...
                // 페이셜 트래킹은 FacialTrackingManager에서 자동으로 처리됨
                Debug.Log("Client: Facial tracking will be managed by FacialTrackingManager");
        ...
        private void UpdateHostData()
        {
            ...
            // Host: 페이셜 트래킹 데이터 수집 및 네트워크 전송
            UpdateHostFacialData();
        }
        ...
        {
            // 클라이언트용 페이셜 트래킹 컴포넌트 사용
            if (clientFacialTracking != null && clientFacialTracking.enabled)
            {
                // 클라이언트에서 받은 페이셜 데이터 적용
                clientFacialTracking.SetFacialDataImmediate(FacialJaw, FacialSmile, FacialWide, FacialO, FacialSad, FacialTongue);
            ...
            else
            {
                if (dataUpdateCount % 600 == 0) // 10초마다
                {
                    Debug.LogWarning("Client: No facial tracking component found or not enabled");
                }
            }
        }
        ...
        private void DebugNetworkData()
        {
            ...
            if (isHost)
            {
                Debug.Log($"Host Facial Component: {(hostFacialTracking != null ? $"Found, Enabled={hostFacialTracking.enabled}" : "NULL")}");
            }
            else
            {
                Debug.Log($"Client Facial Component: {(clientFacialTracking != null ? $"Found, Enabled={clientFacialTracking.enabled}" : "NULL")}");
            }
            ...
            if (isHost)
            {
                ...
                if (hostFacialTracking != null)
                {
                    Debug.Log($"=== HOST FACIAL STATUS ===");
                    Debug.Log($"Facial Tracking Active: {hostFacialTracking.IsFacialTrackingActive()}");
                }
            }
        ...
        private void AttemptAutoRecovery()
        {
            ...
            if (isHost)
            {
                ...
                if (hostFacialTracking == null)
                {
                    ...
                    hostFacialTracking = GetComponentInChildren<SimpleShinanoFacialTracking_Host>();
                    if (hostFacialTracking != null)
                    {
                        Debug.Log($"Host: Facial tracking component found: {hostFacialTracking.name}");
                    }
                }
            }
            else
            {
                // 클라이언트: 페이셜 컴포넌트 재검색
                if (clientFacialTracking == null)
                {
                    Debug.Log("Client: Re-finding facial tracking component...");
                    clientFacialTracking = GetComponentInChildren<SimpleShinanoFacialTracking_Client>();
                    if (clientFacialTracking != null)
                    {
                        Debug.Log($"Client: Facial tracking component found: {clientFacialTracking.name}");
                    }
                }
            }
        ...
        /// <summary>
        /// Host에서 페이셜 트래킹 데이터를 수집해서 네트워크 변수에 설정
        /// </summary>
        private void UpdateHostFacialData()
        {
            if (hostFacialTracking != null && hostFacialTracking.enabled)
            {
                // Host 컴포넌트에서 페이셜 데이터 가져오기
                hostFacialTracking.GetFacialData(
                    out float jaw,
                    out float smile, 
                    out float wide,
                    out float o,
                ...
                // 디버그: 5초마다 페이셜 데이터 송신 로그
                if (dataUpdateCount % 300 == 0 && (jaw > 0.01f || smile > 0.01f || wide > 0.01f || o > 0.01f || sad > 0.01f || tongue > 0.01f))
                {
            ...
            {
                // 페이셜 트래킹이 비활성화되어 있으면 모든 값을 0으로 설정
                if (dataUpdateCount % 600 == 0) // 10초마다
                {
                    if (hostFacialTracking == null)
                    {
                        Debug.LogWarning("Host: hostFacialTracking component is null - facial data will be zero");
                    }
                    else if (!hostFacialTracking.enabled)
                    {
                        Debug.LogWarning("Host: hostFacialTracking is disabled - facial data will be zero");
                    }
                }
        ...
        /// <summary>
        /// 새로운 페이셜 트래킹 시스템 초기화
        /// Host/Client에 따라 적절한 컴포넌트를 찾아서 설정
        /// </summary>
        private void InitializeFacialTracking()
        {
            // Host/Client 컴포넌트 자동 검색
            if (hostFacialTracking == null)
                hostFacialTracking = GetComponentInChildren<SimpleShinanoFacialTracking_Host>();
            ...
            if (isHost)
            {
                Debug.Log($"Host Facial Tracking: {(hostFacialTracking != null ? "Found" : "Not Found")}");
                if (hostFacialTracking != null)
                {
                    Debug.Log($"Host Facial Tracking Enabled: {hostFacialTracking.enabled}");
                }
            }
            else
            {
                Debug.Log($"Client Facial Tracking: {(clientFacialTracking != null ? "Found" : "Not Found")}");
                if (clientFacialTracking != null)
                {
                    Debug.Log($"Client Facial Tracking Enabled: {clientFacialTracking.enabled}");
                }
            }
        }
    }
